...
from my_app.models import UserProfile, IshchiGuruh ,Otryad,WorkHistory
...
def calculate_distance(lat1, lon1, lat2, lon2):
    ...
    # Darajalarni radianlarga o'tkazish
    ...
    return distance
# --- 1. Ishni tugatganda tarixga saqlash ---
def toggle_work(request):
    if request.method == "POST":
        user = UserProfile.objects.filter(login=request.session.get('user_login')).first()
        if not user: return JsonResponse({'status': 'error'}, status=404)

        action = request.POST.get('action')

        if action == 'start':
            user.is_working = True
            user.distance_km = 0.0
            ...
            if user.is_working:
                ...
                WorkHistory.objects.create(
                    user=user,
                    start_time=user.work_start_time or timezone.now(),
                    end_time=timezone.now(),
                    route_data=json.dumps([]),  # Hozircha bo'sh, update_location to'ldiradi
...
def view_history_map(request, history_id):
    # 1. Bazadan yozuvni topish, agar topilmasa 404 xatosini qaytaradi
    history_record = get_object_or_404(WorkHistory, id=history_id)

    # 2. JSON ko'rinishidagi marshrut ma'lumotlarini yuklash
    try:
        route_data = json.loads(history_record.route_data)
    except json.JSONDecodeError:
        route_data = []  # Agar xato bo'lsa, bo'sh ro'yxat

    # 3. Xarita markazini aniqlash (birinchi nuqta yoki default Toshkent)
    if route_data and len(route_data) > 0:
        center_lat = route_data[0][0]
        center_lng = route_data[0][1]
    else:
        center_lat = 41.311081  # Default lat (Toshkent)
        center_lng = 69.240562  # Default lng

    # 4. Leaflet.js xaritasi bilan HTML sahifani yaratish
    html = f"""
    <!DOCTYPE html>
    <html lang="uz">
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Marshrut: {history_record.user.full_name}</title>
    ...
    <body>
        <a href="/statistics/" class="back-btn">⬅ Orqaga</a>

        <div id="map"></div>

        <div class="info-panel">
            <b>Ishchi:</b> {history_record.user.full_name}<br>
            <b>Sana:</b> {history_record.start_time.strftime('%Y-%m-%d')}<br>
            <b>Masofa:</b> {round(history_record.distance_km, 2)} km
        </div>
        ...
        <script>
            ...
            // Marshrut nuqtalari
            var route = {json.dumps(route_data)};

            if (route.length > 0) {{
                // Chiziq chizish (Polylines)
                var polyline = L.polyline(route, {{color: 'blue', weight: 4, opacity: 0.7}}).addTo(map);

                // Boshlanish va tugash markerlari
                var startMarker = L.marker(route[0]).addTo(map).bindPopup("<b>Boshlanish</b><br>{history_record.start_time.strftime('%H:%M')}");
                var endMarker = L.marker(route[route.length-1]).addTo(map).bindPopup("<b>Tugash</b><br>{history_record.end_time.strftime('%H:%M')}");

                // Xaritani marshrutga moslab yaqinlashtirish
                map.fitBounds(polyline.getBounds());
            }} else {{
                alert("Ushbu kun uchun marshrut ma'lumotlari mavjud emas.");
            }}
        </script>
    </body>
    </html>
...
def update_location(request):
    if request.method == "POST":
        ...
        if user and user.is_working:
            # 1. Frontenddan kelgan koordinatalar
            lat = float(request.POST.get('lat', 0.0))
            lng = float(request.POST.get('lng', 0.0))

            # 2. Foydalanuvchining joriy koordinatalarini yangilash
            user.latitude = lat
            user.longitude = lng
            user.save()

            # 3. Marshrut tarixini yangilash
            # Joriy ish vaqti uchun WorkHistory yozuvini topamiz
            last_history = WorkHistory.objects.filter(
                user=user,
                end_time__isnull=True  # Agar end_time bo'sh bo'lsa, joriy ish davri
            ).order_by('-start_time').first()

            # Agar last_history yo'q bo'lsa, yangi yaratish kerak (toggle_work da qilish tavsiya etiladi)
            if last_history:
                route = json.loads(last_history.route_data)
                route.append([lat, lng])
                last_history.route_data = json.dumps(route)

                # Masofani hisoblash (bu yerda calculate_distance funksiyasidan foydalaning)
                # last_history.distance_km += ...
...
# --- 2. Statistika sahifasi (Boss uchun) ---
def statistics_view(request):
    # Barcha ishchilarning tarixini olish
    history = WorkHistory.objects.all().order_by('-start_time')
    ...
    <head>
        ...
        <style>
            ...
            .header {{ color: #00f2ff; text-align: center; margin-bottom: 20px; }}
            .back-btn {{ color: #fff; text-decoration: none; display: block; margin-bottom: 15px; }}
            .history-card {{
                background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1);                
                border-radius: 15px; padding: 15px; margin-bottom: 10px;
            }}
            .worker-name {{ color: #00ff88; font-weight: bold; }}
            .info {{ font-size: 14px; color: #ccc; margin-top: 5px; }}
            .map-link {{
                display: inline-block; margin-top: 10px; color: #000; background: #00f2ff;
                padding: 8px 15px; border-radius: 10px; text-decoration: none; font-weight: bold;
            }}
        </style>
    </head>
    <body>
        <a href="/bosspage/" class="back-btn"><i class="fas fa-arrow-left"></i> Orqaga</a>
        <h2 class="header">ISHCHILAR TARIXI</h2>

        {"".join([f'''
        <div class="history-card">
            <div class="worker-name">{h.user.full_name or h.user.login}</div>
            <div class="info">
                <i class="fas fa-calendar-alt"></i> {h.start_time.strftime('%Y-%m-%d %H:%M')} <br>
                <i class="fas fa-road"></i> {round(h.distance_km, 2)} KM
            </div>
            <a href="/view-history/{h.id}/" class="map-link">Yo'lni ko'rish</a>
        </div>
        ''' for h in history])}
    </body>
    </html>
    """
    return HttpResponse(html)
# --- 2. Joylashuvni yangilash va masofani hisoblash ---



# --- 3. Boss API (Masofani ham yuboradi) ---
def get_single_location(request, worker_id):
    ...
    if worker:
        return JsonResponse({
            'lat': worker.lat,
            'lng': worker.lng,
            'route_list': json.loads(worker.route_data),
...
def track_worker(request, worker_id):
    ...
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Kuzatuv: {worker.login}</title>
        <link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css" />
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
        <style>
    ...
    <head>
        ...
        <style>
            :root {{
                --accent: #00f2ff;
                --boss-red: #ff4747;
                --stats-orange: #ff9800; /* Statistika uchun yangi rang */
            ...
            /* Statistika Tugmasi */
            .stats-btn i {{ color: var(--stats-orange); }}
    ...
    <body>
        ...
        <div class="container">
            ...
            <div class="menu-grid">
                ...
                <a href="/statistics/" class="card stats-btn">
