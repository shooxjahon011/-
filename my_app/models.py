...
class LocationHistory(models.Model):
    ...
    # is_archived maydoni har doim ham kerak emas, lekin qoldirildi

    class Meta:
        ordering = ['-timestamp']

    # 5. Statistika uchun Tarix modeli (Ish yakunlanganda saqlanadi)


class WorkHistory(models.Model):
    user = models.ForeignKey(UserProfile, on_delete=models.CASCADE, related_name='work_history')
    start_time = models.DateTimeField()
    end_time = models.DateTimeField()
    # Marshrut nuqtalarini JSON string ko'rinishida saqlaymiz
    route_data = models.TextField(default="[]")
    distance_km = models.FloatField(default=0.0)

    def __str__(self):
        return f"{self.user.login} - {self.start_time.date()}"
